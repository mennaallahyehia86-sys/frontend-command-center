    ...
    <div dir="rtl" className="app-shell">
      ...
      <main className="main-content">
        ...
        <div className="content-wrap">
          ...
          <section className="bottom-grid"><div className="activity-panel panel"><div className="panel-heading"><div><p className="section-kicker">سجل العمل</p><h2>آخر التحديثات</h2></div><button className="more-button">هذا الأسبوع <ChevronDown size={15} /></button></div><div className="activity-list">{activity.map(([title, repo, time, color]) => <div className="activity-item" key={title}><span className={`activity-dot ${color}`} /><div><strong>{title}</strong><small>{repo}</small></div><time>{time}</time><CheckCircle2 size={16} className="activity-check" /></div>)}</div></div><div className="quality-panel panel"><div className="quality-head"><div><p className="section-kicker">فحص الجودة</p><h2>جاهزية العرض</h2></div><span className="quality-score">٨.٤ <small>/ ١٠</small></span></div><div className="quality-bar"><span style={{ width: "84%" }} /></div><p>أنتِ على بعد <strong>خطوتين</strong> من ملف أعمال مكتمل للمراجعة.</p><div className="quality-tasks"><span><CheckCircle2 size={15} /> README</span><span><CheckCircle2 size={15} /> Responsive</span><span className="pending"><CircleDot size={15} /> Tests</span></div><button className="subtle-button" onClick={() => toast("تم فتح قائمة تحسينات الجودة")}><Settings2 size={15} /> مراجعة التحسينات</button></div></section>
        </div>
      </main>
    </div>
  );
